...
class IpnController extends Controller
{
  ...
  public function actionTestRulesEngineResponse(){
    ...
    */
    ...
    // die (json_encode(['success'=>true,'message'=>"Response of server is: ".$result]));

    $analisi = (array) json_decode($result);
     $save->WriteLog('dashboard','ipn','send',"Analisi is: <pre>".print_r($analisi,true)."</pre>");
    //
    if (is_object($analisi)){
      if (!isset($analisi->errors)){
        if ($analisi->event->group->total_price_eq[0] == 'ok'){
          echo CJSON::encode(['success'=>true,'message'=>'Payload sent!']);
        }else{
          echo CJSON::encode(['success'=>true,'message'=>'Payload sent, but cannot trigger event!']);
        }
      }else{
        echo CJSON::encode(['success'=>true,'message'=>$analisi->errors->detail]);
      }

    }else{
      echo CJSON::encode(['success'=>false,'message'=>'Payload not sent! Retry again.']);
    }


    // echo CJSON::encode(['result'=>json_decode($result)]);
